...
- **svg-icons.html**
    ...
    - Key Terms
      - ```liquid
        {% if %} ... {% endif %}
        ```
        Liquid 템플릿 언어의 조건문으로, 특정 조건이 참일 때만 내부 코드를 실행합니다.
      - ```html
        <a>
        ```
        HTML의 앵커 태그로, 링크를 생성하는 요소입니다.
      - ```html
        href
        ```
        앵커 태그의 속성으로, 링크의 URL을 지정합니다.
      - ```html
        <i>
        ```
        아이콘을 나타내기 위해 사용되는 HTML 요소입니다. 보통 폰트 아이콘 라이브러리와 함께 사용됩니다.
